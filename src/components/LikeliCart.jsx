...
const LikeliCart = ({ data }) => {
  useEffect(() => {
    const max = 5;
    const filteredData = data.filter((d) => d.likelihood !== null);
    ...
    d3.select("#likeli-chart").selectAll("*").remove();
    ...
    // Extract likelihood values from the filtered data
    const likelihoodValues = filteredData.map((d) => d.likelihood);
    const min = 0;

    // Calculate the bin width based on the maximum likelihood and the number of desired bins (5)
    const binWidth = (max - min) / 5;
    ...
    const bins = histogram(likelihoodValues);
    ...
    // Create the y-scale
    const yScale = d3.scaleLinear().domain([0, 450]).nice().range([height, 0]);
    ...
    svg
      ...
      .text("Number of News Likelihood-wise")
  ...
  return (
    <div>
      <h1>Likelihood Histogram</h1>
      <svg id="likeli-chart"></svg>
    </div>
  );
};

export default LikeliCart;
